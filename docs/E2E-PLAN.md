...
実サービス用の接続実装は完了した。[初回実行34861663237](https://github.com/lycanthr0pes/IE_Event_Bot_fork/actions/runs/34861663237)はprepareで失敗し、回収後 `failed_clean`・全資源 `dirty=false` を確認した。KV欠損値の正規化漏れをローカルで再現・修正し、実環境で再確認する。全Calendarを取得しても適用対象は所有2件へ限定する。任意イベントの全件適用、外部APIの全失敗分岐・途中再試行は引き続き未検証である。
