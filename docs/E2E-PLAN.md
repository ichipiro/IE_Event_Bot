...
- [ ] 専用環境の `sync_faults` を実行し、固定注入モデルと実KV書込み・別HTTP読戻しを照合する。外部成功は代替runnerであり、実サービス障害・実KV伝播遅延の観測とは分ける。
...
- [ ] 専用環境で残件回復を再検証し、両キーとも古い場合・旧形式データ・重複適用・通知の一度限り配信・期限確認と書込み間の競合を含め、保証範囲を確定する。
