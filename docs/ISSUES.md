...
- 対応済み範囲（実環境確認待ち）: Webhook simulationは、専用Google eventとNotion pageを同じ強整合manifestで所有し、通常同期と共通のGoogle差分取得とdispatchを通した後、取得結果のevent IDとrun markerが一致する1件だけを適用する専用scenarioを実装した。同期cursor、最終実行時刻、最終結果、Google認証cache、対応表、queueは実行内へ閉じ込める。実Webhook配信、watch、token、重複抑止、実Cronは対象外である。
