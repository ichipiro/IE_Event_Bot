...
- 部分失敗の修正: Google→Discordの作成・更新失敗でIDを得られない場合に、残件を保存せず成功扱いになる不具合を2ケースで再現・修正した。所有2件E2Eへ固定失敗・部分反映読戻し・次回queue処理を追加してローカル検証した。実サービス上の固定注入シナリオは未実行。削除失敗・Notion ID書戻し失敗・応答喪失後の重複は別の未検証範囲。
