...
- 対応中範囲: QA通知は、専用Notion Q&A pageとDiscord messageを同じ強整合manifestで所有し、通常ジョブと共通の初回抑止・更新通知処理を1件だけへ適用して自己cleanupする専用scenarioを実装した。共有 `qa_cache`、Q&A DB全件取得、質問番号補完は対象外であり、実サービス確認は未実施である。
- 未対応範囲: Google / Discord 差分取得と cursor / snapshot / queue、全体同期、Webhook simulation、実 webhook / Cron、通常QAジョブの全件取得と共有cache、前日リマインド、Notion cleanupが変更する下流資源と状態には run ID が伝播していない。
- 暫定対応: 未対応の通常同期、Webhook simulation、通常ジョブrouteは `E2E_ORCHESTRATED_WRITES_ENABLED=false` で `404` にする。read-only preflight、service CRUD、所有資源限定のサービス間scenarioとQA通知scenarioは別routeで継続する。
