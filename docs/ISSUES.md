...
- 対応済み範囲（実環境確認待ち）: 前日リマインドは、専用 Discord Scheduled Event と message を同じ強整合 manifest で所有し、通常ジョブと共通の通知ウィンドウ判定と重複抑止を1件だけへ適用して自己 cleanup する専用 scenario を実装した。共有 `reminder_cache`、Guild の通常 event 一覧処理、実 Cron は対象外であり、実サービス workflow は未実施である。
- 未対応範囲: Google / Discord 差分取得と cursor / snapshot / queue、全体同期、Webhook simulation、実 webhook / Cron、通常QAジョブの全件取得と共有cache、通常リマインドのGuild全件取得と共有cache、Notion cleanupが変更する下流資源と状態には run ID が伝播していない。
- 暫定対応: 未対応の通常同期、Webhook simulation、通常ジョブ route は `E2E_ORCHESTRATED_WRITES_ENABLED=false` で `404` にする。read-only preflight、service CRUD、所有資源限定のサービス間 scenario、QA通知 scenario、前日リマインド scenario は別 route で継続する。
