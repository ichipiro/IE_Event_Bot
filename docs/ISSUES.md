...
- 追加実装: MCPで最初のadvanceのHTTP 200ヘッダー受信後に本文を未読のまま破棄する。更新完了checkpointを別取得し、再deploy後の再送・続行へ進む。注入flagと固定エラーを監査に残し、通信失敗や非200応答を注入成功にしない。実サービス検証はこの時点では未実施。回線断やWorker停止の再現は含まない。
