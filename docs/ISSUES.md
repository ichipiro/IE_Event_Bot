...
- Calendar診断: [実行35965137690](https://github.com/lycanthr0pes/IE_Event_Bot_fork/actions/runs/35965137690)はHTTP 200・`calendar_deleted`で成功し、通常予定がなく削除履歴だけ残っていることを確認した。予定・KVへの書込みはなく、全manifestは前回と同一で `dirty=false`。開始時の削除履歴を変更不可のハッシュで記録し、一致する履歴だけを除外する修正を追加した。通常予定・未知の履歴・変更された履歴は拒否する。同じCalendarでの全件適用は再検証待ち。
