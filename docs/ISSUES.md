...
- 対応済み範囲: Discord→Notion は、専用 Discord Scheduled Event と Notion page を同じ強整合 manifest で所有し、既存の `_sync_discord_event_upsert` を通した検証と自己 cleanup を行う専用 scenario を実装した。Google、外部 Notion DB、通常の Discord snapshot / queue、作成通知は変更対象にしない。実サービスでの実行は未確認である。
- 未対応範囲: Discord→Google、Google / Discord 差分取得と cursor / snapshot / queue、全体同期、Webhook simulation、実 webhook / Cron、定期ジョブが変更する下流資源と状態には run ID が伝播していない。
- 暫定対応: 未対応の通常同期、Webhook simulation、ジョブは `E2E_ORCHESTRATED_WRITES_ENABLED=false` で `404` にする。read-only preflight、service CRUD、所有資源限定の Google→Notion / Discord と Discord→Notion scenario は別 route で継続する。
