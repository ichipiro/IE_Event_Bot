...
- Calendar診断: [実行35965137690](https://github.com/lycanthr0pes/IE_Event_Bot_fork/actions/runs/35965137690)はHTTP 200・`calendar_deleted`で成功し、通常予定がなく削除履歴だけ残っていることを確認した。予定・KVへの書込みはなく、全manifestは前回と同一で `dirty=false`。現行の空状態ガードを満たすには、削除履歴のない新規E2E専用Calendarへの切替が必要。全件適用は引き続き未検証。
