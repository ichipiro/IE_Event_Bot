...
- `trigger_job` の `qa_check` と `reminder` は通常の `/jobs/*` ではなく、所有資源限定の `/admin/e2e/qa-notification` と `/admin/e2e/reminder` を呼ぶ。Notion cleanup と run-all は通常 route のまま既定拒否を維持する。
