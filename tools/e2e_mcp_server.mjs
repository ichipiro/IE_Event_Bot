...
export async function appendAuditEntry(entry) {
  ...
  const safeEntry = {
    ...
    tool: entry.tool,
    target: entry.target,
    sync_phase: ["run", "prepare", "prepare_full", "prepare_boundary", "prepare_matrix", "prepare_all", "prepare_http", "prepare_webhook", "webhook_trigger", "http_advance", "inspect", "advance", "resume"].includes(entry.sync_phase) ? entry.sync_phase : null,
    phase: entry.phase,
    ok: Boolean(entry.ok),
  ...
  try {
    response = await fetchImpl(`${config.workerUrl}${route}`, {
      ...
      signal: AbortSignal.timeout(
        method === "POST" &&
        (route === "/sync/all" || ["", "/full", "/boundary", "/matrix", "/all", "/http", "/watch", "/watch/trigger", "/advance", "/verify", "/cleanup"].some(suffix => route === `${SCENARIO_ROUTES.google_sync}${suffix}`))
  ...
  server.registerTool(
    "trigger_sync",
    {
      ...
      inputSchema: {
        ...
        sync_phase: z.enum(["run", "prepare", "prepare_full", "prepare_boundary", "prepare_matrix", "prepare_all", "prepare_http", "prepare_webhook", "webhook_trigger", "http_advance", "inspect", "advance", "resume"]).default("run"),
    ...
    async ({ run_id: runId, scenario, sync_phase: syncPhase, version_sha256: versionSha256, response_mode: responseMode }) => {
      ...
      if ((["prepare_full", "prepare_boundary", "prepare_matrix", "prepare_all", "prepare_http", "prepare_webhook", "webhook_trigger", "http_advance", "inspect"].includes(syncPhase) && scenario !== "google_sync") ||
